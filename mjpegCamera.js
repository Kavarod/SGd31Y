...
Camera.prototype.start = function() {
  let videostream = this._getVideoStream();
    //* Add this code from above under the if section
  videostream.on('data',(data)=>{this.onFrame(data);this.emit("frame",data.toString("base64"));});
  if (this.motion) {
     //! IF you want to stream movion videos!!!!
...
Camera.prototype.getScreenshot = function(callback) {
  if (this.connection) {
    ...
    let videostream = this._getVideoStream(handleError);
